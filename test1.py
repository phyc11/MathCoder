    ...
    for tap in [1, 2]:  
        for start in range(1, 100):
            for page in range(1, 150):
                url = f"https://vietjack.com/sbt-toan-{lop}-cd/bai-{start}-trang-{page}-sbt-toan-lop-{lop}-tap-{tap}-canh-dieu.jsp"
                urls.append(url)
...
if __name__ == "__main__":
    for lop in [7, 10]: 
        ...
        with open(f"Link{lop}/valid_links_math{lop}_cd.txt", "w", encoding="utf-8") as f_links:
            for link in links:
                f_links.write(link + "\n")
        ...
        with open(f"Link{lop}/skipped_links_math{lop}_cd.txt", 'w', encoding='utf-8') as f:
