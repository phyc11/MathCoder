    ...
    for tap in [1, 2]:  
        for start in range(1, 30):
            for page in range(1, 150):
                url = f"https://vietjack.com/sbt-toan-{lop}-ct/bai-{start}-trang-{page}-sbt-toan-lop-{lop}-tap-{tap}.jsp"
                urls.append(url)
...
if __name__ == "__main__":
    for lop in [10]: 
        ...
        with open(f"Link{lop}/valid_links_math{lop}_ct.txt", "a", encoding="utf-8") as f_links:
            for link in links:
                f_links.write(link + "\n")
        ...
        with open(f"Math{lop}/raw/Math{lop}_raw.jsonl", "a", encoding="utf-8") as f:
            for item in all_data:
                f.write(json.dumps(item, ensure_ascii=False) + "\n")

        
        with open(f"Link{lop}/skipped_links_math{lop}_ct.txt", 'a', encoding='utf-8') as f:
            for link in skipped_links:
                f.write(link + '\n')

        print(f"Đã lưu {len(all_data)} bài cho lớp {lop} vào Math{lop}_ct_raw.jsonl")

# if __name__ == "__main__":
#     with open('Link9/valid_links_math9_cd.txt', 'r', encoding='utf-8') as file:
#         links = [line.strip() for line in file if line.strip()]

#     print(f"🔎 Đang lấy dữ liệu từ {len(links)} link...")
#     all_data, skipped_links = extract_all_data(links)

#     with open('Math9/raw/Math9_raw.jsonl', 'a', encoding='utf-8') as f:
#         for item in all_data:
